...
DECLARE_SCRIPT(MPR_A01_Chinook_Evac_Civies_PMP,"")
{
	...
	void Created(GameObject* obj)
	{
		...
		Commands->Set_Animation( obj, "V_GDI_CHINOOKE3.m_ch_evacland", false, NULL, 0.0f, -1.0f, true);
	}
	...
	void Animation_Complete(GameObject* obj, const char* name)
	{
		...
		{
			Commands->Set_Animation( obj, "V_GDI_CHINOOKE3.m_ch_evacloiter", true, NULL, 0.0f, -1.0f, true);
	...
	void Timer_Expired (GameObject* obj, int Timer_ID)
	{
		...
		{
			Commands->Set_Animation( obj, "V_GDI_CHINOOKE3.m_ch_evacoff", false, NULL, 0.0f, -1.0f, true);
		}
	}
};
...
DECLARE_SCRIPT(MPR_A03_Bridge_Exploding_DEL, "AnimationName=:string")
{
	...
	void Custom(GameObject* game_obj, int type, int, GameObject*)
	{
		...
		if (type == MPR_CUSTOM_EXPLODE_BRIDGE_DEL) {
			const char* animName = Get_Parameter("AnimationName");
			Commands->Set_Animation(game_obj, animName, false, NULL, 0.0f, -1.0f, true);
		}
	}
};
...
//			Vector3 extent = Commands->Get_Extent(tiberium);		// See Byon for a new way to do this, if needed
	...
	void Harvest(void)
	{
		...
		{
			Commands->Set_Animation(Owner(), "v_nod_harvester.m_harvesting", true, NULL, 0.0f, -1.0f, true);
	...
	void Entered(GameObject*, GameObject* enterer)
	{
		...
		if (enterer == star)
		{
			...
			if (apache != NULL)
			{
				...
				Commands->Send_Custom_Event(Owner(), apache, MPR_CUSTOM_APACHE_GOTO_POSITION_DEL, position, 0);
			}
		}
	}
};
...
DECLARE_SCRIPT(MPR_ApacheShootAtPlayer, "ApacheID:int")
{
	void Entered(GameObject*, GameObject* enterer)
	{
		...
		if (enterer == star)
		{
			...
			if (apache != NULL)
			{
				Commands->Send_Custom_Event(Owner(), apache, MPR_CUSTOM_APACHE_SHOOT_DEL, (int)true, 0);
			}
		}
	}
	...
	void Exited(GameObject*, GameObject* exiter)
	{
		...
		if (exiter == star)
		{
			...
			if (apache != NULL)
			{
				Commands->Send_Custom_Event(Owner(), apache, MPR_CUSTOM_APACHE_SHOOT_DEL, (int)false, 0);
			}
		}
	}
};
...
DECLARE_SCRIPT(MPR_ApacheController, "")
{
	...
	void Created(GameObject*)
	{
		...
		//David Y.
		//Commands->Set_Animation(Owner(), "V_APACHE-E3.M_AP_POSITION1", true, NULL, 0.0f, -1.0f, true);
		// Rotor anim fix

		Vector3 origin(0.0f, 0.0f, 0.0f);
		Commands->Set_Position(Owner(), origin);
		Commands->Set_Facing(Owner(), 0.0f);
		Commands->Set_Animation(Owner(), "M_AP_POSITION1", true, NULL, 0.0f, -1.0f, true);
	...
	void Goto_Position(ApachePosition position)
	{
		...
		if ((position != mCurrentPosition) || (position != mDesiredPosition))
		{
			...
			Commands->Set_Animation(Owner(), transition.TransitionAnim, false, NULL, 0.0f, -1.0f, true);
		}
	}
	...
	void Animation_Complete(GameObject* apache, const char* finishedAnim)
	{
		...
		if (stricmp(animName, transition.TransitionAnim) == 0)
		{
			...
			Commands->Set_Animation(Owner(), transition.HoverAnim, true, NULL, 0.0f, -1.0f, true);
		}
	}
...
DECLARE_SCRIPT(MPR_A01_Orca_Strike_On_Turret_RAD, "Target_Number:int, AnimationName:string, Fire_01:int, Fire_02:int, Fire_03:int, Fire_04:int, Fire_05:int, Fire_06:int")
{
	...
	void Custom(GameObject* p_obj, int n_type, int n_param, GameObject* p_sender)
	{
		if (n_type == MPR_A01_CUSTOM_ORCA_TURRETSTRIKE_RAD)
		{
			...
			v_target_loc.Z += 0.5f;
			Commands->Set_Animation(p_obj, Get_Parameter("AnimationName"), 0, NULL, 0.0f, -1.0f, true);
...
DECLARE_SCRIPT(MPR_A04_Main_Control_Terminal_RAD, "ObeliskControllerID:int, BuildingID:int")
{
	void Killed(GameObject* p_obj, GameObject* p_killer)
	{
		...
		{
			Commands->Send_Custom_Event(p_obj, Commands->Find_Object(Get_Int_Parameter("ObeliskControllerID")), MPR_A04_CUSTOM_OBELISK_FIRING_RAD, 0, 0);
		}
		else
		{
...
DECLARE_SCRIPT(MPR_A05_Tank_Controller_RAD, "My_Number:int, MammothTank:int")
{
	...
	void Created(GameObject* p_obj)
	{
		...
		if (n_mynumber == 2)
		{
			...
			{
				Commands->Send_Custom_Event(p_obj, Commands->Find_Object(n_mammoth), MPR_A05_CUSTOM_MAMMOTH_RAD, Commands->Get_ID(p_obj), 0);
			}
		}
	}
...
DECLARE_SCRIPT(MPR_A05_C130_Dropoff_Animation_RMV, "Cinematic=:string,ObjToCreate=:string,DropFrame=:int,BoneName=:string")
{
	...
	void Created( GameObject * obj )
	{
		Commands->Set_Animation ( obj, Get_Parameter( "Cinematic" ), 0 , NULL, 0.0f, -1.0f, true);
...
DECLARE_SCRIPT(MPR_A05_A10_Air_Strike_RMV, "") 
{
	...
	void Timer_Expired ( GameObject* obj, int timer_id )
	{
		if ( timer_id == GO )
		{
			Commands->Set_Animation( obj, "v_gdi-a10e3.m_a10strike_a", 0 , NULL, 0.0f, -1.0f, true);
		}
		if ( timer_id == SHOOT )
		{
...
DECLARE_SCRIPT(MPR_A01_Intro_Cinematic_RMV, "")
{
	...
	void Timer_Expired(GameObject * obj, int timer_id)
	{
		...
		if ( timer_id == INTRO )
		{
			...
			Commands->Set_Animation( Commands->Get_The_Star(), "s_a_human.m_havok_e3pose", 0 , NULL, 0.0f, -1.0f, true);
...
DECLARE_SCRIPT(MPR_A02_Drill_Instructor_JDG, "")
{
	...
	void Killed (GameObject* obj, GameObject* Killer)
	{
		...
		{
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_cadet_1_id_JDG)), MPR_A02_CUSTOM_CADET_1_JDG, MPR_A02_PARAM_CADETS_GOTO_INNATE_JDG, 0);
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), MPR_A02_CUSTOM_CADET_2_JDG, MPR_A02_PARAM_CADETS_GOTO_INNATE_JDG, 0);
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), MPR_A02_CUSTOM_CADET_3_JDG, MPR_A02_PARAM_CADETS_GOTO_INNATE_JDG, 0);
		}
	}
	...
	void Custom (GameObject* obj, int Type, int Param, GameObject* Sender)
	{
		...
		if ((Type == MPR_A02_CUSTOM_CADET_1_JDG) && (Param == MPR_A02_PARAM_CADETS_AT_ATTENTION_JDG))
		{
			...
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_J04C", 0, NULL, 0.0f, -1.0f, true);
	...
	void Ten_Hut (GameObject* obj)
	{
		...
		if (Commands->Find_Object(MPR_A02_cadet_2_id_JDG))
		{
			...
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_J18C",0, NULL, 0.0f, -1.0f, true);
	...
	void Choose_Cadet (void)
	{
		...
		if ((cadet > 0) && (cadet <= 33))
		{
			if (Commands->Find_Object(MPR_A02_cadet_1_id_JDG))
			{
				...
				Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_1_id_JDG)), "S_A_HUMAN.H_A_J09C", 0, NULL, 0.0f, -1.0f, true);
		...
		else if ((cadet > 33) && (cadet <= 66))
		{
			if (Commands->Find_Object(MPR_A02_cadet_2_id_JDG))
			{
				...
				Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), "S_A_HUMAN.H_A_J09C", 0, NULL, 0.0f, -1.0f, true);
		...
		else
		{
			if (Commands->Find_Object(MPR_A02_cadet_3_id_JDG))
			{
				...
				Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), "S_A_HUMAN.H_A_J09C", 0, NULL, 0.0f, -1.0f, true);
	...
	void Rag_On_Cadet (int Cadet_Number)
	{
		...
		Commands->Set_Animation ((Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), "S_A_HUMAN.H_A_J07C", 0, NULL, 0.0f, -1.0f, true);
	...
	{	
		...
		if ((stricmp("S_A_HUMAN.H_A_DIV1", name) == 0) && (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)) && (state == DI_INNATE))
		{
			...
			//Commands->Set_Animation (obj, "S_A_HUMAN.H_A_C2C0", 1, NULL, 0.0f, -1.0f, true);	
		}
	...
	void Timer_Expired (GameObject* obj, int Timer_ID)
	{
		...
		if ((Timer_ID == MPR_A02_TIMER_DRILL_INSTRUCTOR_ATTACK_DELAY_JDG) && (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)) && (state == DI_INNATE))
		{
			...
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_DIV1", 0, NULL, 0.0f, -1.0f, true);
			//forward roll
		}
	}
	...
	void Next_Exercise (void)
	{
		...
		if ((exercise == PUSHUPS) && (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), "S_A_HUMAN.H_A_J04C", 0, NULL, 0.0f, -1.0f, true);
			//S_A_HUMAN.H_A_J04C is animation for give order
...
DECLARE_SCRIPT(MPR_A02_Drill_Start_Zone_JDG, "")
{
	...
	void Entered (GameObject* obj, GameObject* Enterer)
	{
		...
		{
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), MPR_A02_CUSTOM_DRILL_INSTRUCTOR_JDG, MPR_A02_PARAM_CADETS_TEN_HUT_JDG, 0);
...
DECLARE_SCRIPT(MPR_A02_Drill_Cadet_1_JDG, "")
{
	...
	void Killed (GameObject* obj, GameObject* Killer)
	{
		if (Killer == (Commands->Get_The_Star()))
		{
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), MPR_A02_CUSTOM_DRILL_INSTRUCTOR_JDG, MPR_A02_PARAM_CADETS_GOTO_INNATE_JDG, 0);
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), MPR_A02_CUSTOM_CADET_2_JDG, MPR_A02_PARAM_CADETS_GOTO_INNATE_JDG, 0);
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), MPR_A02_CUSTOM_CADET_3_JDG, MPR_A02_PARAM_CADETS_GOTO_INNATE_JDG, 0);
		}
	}
	...
	void Timer_Expired (GameObject* obj, int Timer_ID)
	{
		if ((Timer_ID == MPR_A02_TIMER_CADET_1_ATTACK_DELAY_JDG) && (state == CADET_1_INNATE))
		{
			...
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_D2A3", 0, NULL, 0.0f, -1.0f, true);
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_1_TEN_HUT_DELAY_JDG) && (state != CADET_1_INNATE))
		{
			...
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_A0A0", 0, NULL, 0.0f, -1.0f, true);
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_1_PUSHUPS_DELAY_JDG) && (state != CADET_1_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_1_id_JDG)), "S_A_HUMAN.H_A_J43C", 0, NULL, 0.0f, -1.0f, true);
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_1_SITUPS_DELAY_JDG) && (state != CADET_1_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_1_id_JDG)), "S_A_HUMAN.H_A_J13C", 0, NULL, 0.0f, -1.0f, true);
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_1_JUMPING_JACKS_DELAY_JDG) && (state != CADET_1_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_1_id_JDG)), "S_A_HUMAN.H_A_J43A" , 0, NULL, 0.0f, -1.0f, true);
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_1_MISTAKE_DELAY_JDG) && (state != CADET_1_INNATE))
		{
			...
			{
				Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_1_id_JDG)), "S_A_HUMAN.H_A_J14C", 1, NULL, 0.0f, -1.0f, true);
				//S_A_HUMAN.H_A_J14C is animation for situps
			}
	...
	void Animation_Complete (GameObject* obj, const char* name)
	{
		if ((stricmp(name, "S_A_HUMAN.H_A_J43C") == 0) && (state != CADET_1_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_1_id_JDG)), "S_A_HUMAN.H_A_J13A", 0, NULL, 0.0f, -1.0f, true);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_J43A") == 0) && (state != CADET_1_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_1_id_JDG)), "S_A_HUMAN.H_A_J43B", 1, NULL, 0.0f, -1.0f, true);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_J13A") == 0) && (state != CADET_1_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_1_id_JDG)), "S_A_HUMAN.H_A_J13B", 1, NULL, 0.0f, -1.0f, true);
		}
		...
		if ((stricmp (name,"S_A_HUMAN.H_A_A0A0") ==0 ) && (state != CADET_1_INNATE))
		{
			...
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), MPR_A02_CUSTOM_CADET_1_JDG, MPR_A02_PARAM_CADETS_AT_ATTENTION_JDG, 0);
		}
		...
		if ((stricmp (name, "S_A_HUMAN.H_A_J09C") == 0 ) && (state != CADET_1_INNATE))
		{
			...
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), MPR_A02_CUSTOM_CADET_1_JDG, MPR_A02_PARAM_CADETS_MADE_MISTAKE_JDG, 0);
...
DECLARE_SCRIPT(MPR_A02_Drill_Cadet_2_JDG, "")
{
	...
	void Timer_Expired (GameObject* obj, int Timer_ID)
	{
		if ((Timer_ID == MPR_A02_TIMER_CADET_2_ATTACK_DELAY_JDG) && (state == CADET_2_INNATE))
		{
			...
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_D2B2", 0, NULL, 0.0f, -1.0f, true);
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_2_PUSHUPS_DELAY_JDG) && (state != CADET_2_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), "S_A_HUMAN.H_A_J43C", 0, NULL, 0.0f, -1.0f, true);
			//S_A_HUMAN.H_A_J13B is animation for pushups
		}
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_2_SITUPS_DELAY_JDG) && (state != CADET_2_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), "S_A_HUMAN.H_A_J13C", 0, NULL, 0.0f, -1.0f, true);
			//S_A_HUMAN.H_A_J14C is animation for situps
		}
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_2_JUMPING_JACKS_DELAY_JDG) && (state != CADET_2_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), "S_A_HUMAN.H_A_J43A" , 0, NULL, 0.0f, -1.0f, true);
			//S_A_HUMAN.H_A_J43B is animation for j-jacks
		}
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_2_MISTAKE_DELAY_JDG) && (state != CADET_2_INNATE))
		{
			...
			{
				Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), "S_A_HUMAN.H_A_J14C", 1, NULL, 0.0f, -1.0f, true);
				//S_A_HUMAN.H_A_J14C is animation for situps
			}
	...
	void Animation_Complete (GameObject* obj, const char* name)
	{
		if ((stricmp(name, "S_A_HUMAN.H_A_J43C") == 0) && (state != CADET_2_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), "S_A_HUMAN.H_A_J13A", 0, NULL, 0.0f, -1.0f, true);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_J43A") == 0) && (state != CADET_2_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), "S_A_HUMAN.H_A_J43B", 1, NULL, 0.0f, -1.0f, true);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_J13A") == 0) && (state != CADET_2_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_2_id_JDG)), "S_A_HUMAN.H_A_J13B", 1, NULL, 0.0f, -1.0f, true);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_A0A0") == 0) && (state != CADET_2_INNATE))
		{
			...
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), MPR_A02_CUSTOM_CADET_2_JDG, MPR_A02_PARAM_CADETS_AT_ATTENTION_JDG, 0);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_J09C") == 0) && (state != CADET_2_INNATE))
		{
			...
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), MPR_A02_CUSTOM_CADET_2_JDG, MPR_A02_PARAM_CADETS_MADE_MISTAKE_JDG, 0);
...
DECLARE_SCRIPT(MPR_A02_Drill_Cadet_3_JDG, "")
{
	...
	void Timer_Expired (GameObject* obj, int Timer_ID)
	{
		if ((Timer_ID == MPR_A02_TIMER_CADET_3_ATTACK_DELAY_JDG) && (state == CADET_3_INNATE))
		{
			...
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_D2A4", 0, NULL, 0.0f, -1.0f, true);
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_3_PUSHUPS_DELAY_JDG) && (state != CADET_3_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), "S_A_HUMAN.H_A_J43C", 0, NULL, 0.0f, -1.0f, true);
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_3_SITUPS_DELAY_JDG) && (state != CADET_3_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), "S_A_HUMAN.H_A_J13C", 0, NULL, 0.0f, -1.0f, true);
			//S_A_HUMAN.H_A_J14C is animation for situps
		}
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_3_JUMPING_JACKS_DELAY_JDG) && (state != CADET_3_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), "S_A_HUMAN.H_A_J43A" , 0, NULL, 0.0f, -1.0f, true);
			//S_A_HUMAN.H_A_J43B is animation for j-jacks
		}
		...
		if ((Timer_ID == MPR_A02_TIMER_CADET_3_MISTAKE_DELAY_JDG) && (state != CADET_3_INNATE))
		{
			...
			{
				Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), "S_A_HUMAN.H_A_J14C", 1, NULL, 0.0f, -1.0f, true);
	...
	void Animation_Complete (GameObject* obj, const char* name)
	{
		if ((stricmp(name, "S_A_HUMAN.H_A_J43C") == 0) && (state != CADET_3_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), "S_A_HUMAN.H_A_J13A", 0, NULL, 0.0f, -1.0f, true);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_J43A") == 0) && (state != CADET_3_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), "S_A_HUMAN.H_A_J43B", 1, NULL, 0.0f, -1.0f, true);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_J13A") == 0) && (state != CADET_3_INNATE))
		{
			...
			Commands->Set_Animation ((Commands->Find_Object(MPR_A02_cadet_3_id_JDG)), "S_A_HUMAN.H_A_J13B", 1, NULL, 0.0f, -1.0f, true);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_A0A0") == 0) && (state != CADET_3_INNATE))
		{
			...
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), MPR_A02_CUSTOM_CADET_3_JDG, MPR_A02_PARAM_CADETS_AT_ATTENTION_JDG, 0);
		}
		...
		if ((stricmp(name, "S_A_HUMAN.H_A_J09C") == 0) && (state != CADET_3_INNATE))
		{
			...
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_drill_instructor_id_JDG)), MPR_A02_CUSTOM_CADET_3_JDG, MPR_A02_PARAM_CADETS_MADE_MISTAKE_JDG, 0);
...
DECLARE_SCRIPT(MPR_A04_SuicideRun_GDI_Minigunner_1_JDG, "")
{
	...
	{	
		...
		{
			Commands->Set_Animation(obj, "S_A_HUMAN.H_A_J33C", 0, NULL, 0.0f, -1.0f, true);
	...
	void Animation_Complete (GameObject* obj, const char* name)
	{
		...
		{
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_J15C", 0, NULL, 0.0f, -1.0f, true);
		}

		if ((stricmp(name, "S_A_HUMAN.H_A_J15C") == 0)) 
		{
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_J03C", 0, NULL, 0.0f, -1.0f, true);
		}
...
DECLARE_SCRIPT(MPR_A04_SuicideRun_GDI_Minigunner_2_JDG, "")
{
	...
	{	
		...
		{
			Commands->Set_Animation(obj, "S_A_HUMAN.H_A_J09C", 0, NULL, 0.0f, -1.0f, true);
		}
	}
	...
	void Animation_Complete (GameObject* obj, const char* name)
	{
		...
		{
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_J10C", 0, NULL, 0.0f, -1.0f, true);
		}

		if ((stricmp(name, "S_A_HUMAN.H_A_J10C") == 0)) 
		{
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_J11C", 0, NULL, 0.0f, -1.0f, true);
		}
...
DECLARE_SCRIPT(MPR_A02_DriveBy_EnterZone_JDG, "")
{
	...
	void Entered (GameObject* obj, GameObject* Enterer)
	{
		if (Enterer == (Commands->Get_The_Star()) && (Havoc_in_this_Zone == 0))
		{
			...
			Commands->Send_Custom_Event (obj, (Commands->Find_Object(MPR_A02_driveby_harvester_id_JDG)), MPR_A02_CUSTOM_DRIVEBY_HARVESTER_JDG, MPR_A00_PARAM_HAVOC_IN_ENTER_ZONE_JDG, 0);
...
DECLARE_SCRIPT(MPR_A03_Helicopter_Injured_NOD_Pilot_JDG, "")
{
	...
	void Created(GameObject* obj)
	{
		...
		Commands->Set_Animation (obj, "S_A_HUMAN.H_A_601A", 1, NULL, 0.0f, -1.0f, true);
	...
	void Timer_Expired (GameObject*obj, int Timer_ID)
	{
		if ((Timer_ID == MPR_A03_TIMER_INJURED_PILOT_CRAWL_1_JDG) && (Commands->Find_Object(MPR_A03_helicopter_injured_nod_pilot_id_JDG)))
		{
			...
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_H11C", 0, NULL, 0.0f, -1.0f, true);
	...
	void Animation_Complete (GameObject* obj, const char* name)
	{
		...
		{
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_611A", 0, NULL, 0.0f, -1.0f, true);
		...
		{
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_J22C", 0, NULL, 0.0f, -1.0f, true);
...
DECLARE_SCRIPT(MPR_A03_BridgeScene_Engineer_JDG, "")
{
	...
	void Timer_Expired (GameObject* obj, int Timer_ID)
	{
		if ((Timer_ID == MPR_A03_TIMER_BRIDGESCENE_ENGINEER_HAVOC_IN_ZONE_DELAY_JDG) && (Commands->Find_Object(MPR_A03_bridgescene_engineer_id_JDG)))
		{
			...
			Commands->Set_Animation (obj, "S_A_HUMAN.H_A_J12C", 0, NULL, 0.0f, -1.0f, true);
