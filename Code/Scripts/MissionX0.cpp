...
DECLARE_SCRIPT(MX0_MissionStart_DME, "") //1200001
{
	...
	{
		Commands->Select_Weapon (STAR, NULL , false);
	...
	void Timer_Expired(GameObject * obj, int timer_id)
	{
		...
		if (timer_id == EQUIP)
		{
			...
			Commands->Give_PowerUp (STAR, "POW_AutoRifle_Player", false);
			Commands->Select_Weapon (STAR, "Weapon_AutoRifle_Player" , false);
...
DECLARE_SCRIPT(MX0_GDI_ORCA, "" )
{
	...
	void Custom( GameObject *obj, int type, int param, GameObject *sender )
	{
		...
		if ( type == 2 )
		{
			...
			int conv_id = Commands->Create_Conversation ( "MX0_A03_04" , 50, 500.0f, true);
...
/*			if ( zone_not_finished )
			{
				...
				if ( rand == 1 )
				{
					...
					int conv_id = Commands->Create_Conversation ( "MX0_A03_06" , 50, 500.0f, true);
					Commands->Join_Conversation( Trooper_One, conv_id, false, false, true);
				...
				else
				{
					...
					int conv_id = Commands->Create_Conversation ( "MX0_A03_07" , 50, 500.0f, true);
					Commands->Join_Conversation( Trooper_One, conv_id, false, false, true);
					Commands->Start_Conversation (conv_id, 0);
				}
			}
		...
		if ( type == MX0_A03_CUSTOM_HARVESTER_DEAD )
		{
			...
			if ( zone_not_finished )
			{
				if (Trooper_One)
				{
					...
					int conv_id = Commands->Create_Conversation ( "MX0_A03_08" , 50, 500.0f, true);
					Commands->Join_Conversation( Trooper_One, conv_id, false, false, true);
					Commands->Start_Conversation (conv_id, 0);
				}
			}
	...
	{		
		...
		if (type == 3)
		{
			if ( Harvester_Not_Destroyed )
			{
				...
				int conv_id = Commands->Create_Conversation ( "MX0_A03_05" , 50, 500.0f, true);
